...
class Music(commands.Cog):
    ...
    async def on_ready(self):
        print("Music Cog has loaded")

    @commands.command(name='join',
                      invoke_without_subcommand=True,
                      help="Joins a voice chanel")
    ...
    @commands.command(name='now', help="Deplays currently playing music")
    ...
    async def _pause(self, ctx: commands.Context):
        ...
        if ctx.voice_state.loop:
            ...
            await ctx.send("Loop mode has been turned off")
    ...
    @commands.command(name='queue', help="Displays queue")
    ...
    @commands.command(name='search', help="Shows top 10 results from youtube")
    async def _search(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            if ctx.voice_state.loop:
                await ctx.send("Turn off loop mode to add more songs")
            try:
                source = await YTDLSource.search_source(self.bot,
                                                        ctx,
                                                        search,
                                                        loop=self.bot.loop)
            except YTDLError as e:
